...
if(isset($db) && $db !== false){
    ...
    && isset($_POST['to_time']) && is_numeric($_POST['to_time'])){
        ...
        $res_room = getRoomsResult($from_time, $to_time, $num_adults, $num_children, true, $room_id);
        
        if(!isset($res_room[$room_id]) || (isset($res_room[$room_id]) && isset($res_room[$room_id]['error']) && !isset($res_room[$room_id]['notice']))){
            $response['html'] .= '<span class="label label-danger"><i class="fa fa-warning"></i> '.$texts['NO_AVAILABILITY'].'</small></span>
            ...
            $response['html'] .= '<span class="label label-danger"><i class="fa fa-warning"></i> <small>'.$res_room[$room_id]['notice'].'</small></span>';
        elseif(isset($res_room[$room_id]) && !isset($res_room[$room_id]['error'])){
            ...
            $response['html'] .= ' <i class="fa fa-check-circle text-success"></i> <a href="#btn-book" class="btn btn-sm"><i class="fa fa-arrow-up"></i> '.$texts['BOOK'].' </a>';
        }
    }
