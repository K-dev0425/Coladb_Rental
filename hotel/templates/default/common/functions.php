...
function getBookedRooms($from_time, $to_time, $room_id = 0)
{
    ...
    if(!empty($room_id)) $query_book .= ' AND r.id = '.$room_id;
    ...
    if($result_book !== false){
        foreach($result_book as $i => $row){
            ...
            $res_room[$id_hotel][$id_room]['room_stock'] = max(0, $room_stock-max($days[$id_room]));
        }
    }
    return $booked;
}
...
function getBookingSummary($room_id = null, $index = null, $amount = 0, $available = true)
{
    ...
    if($total > 0){
        $html .= '
        <big><i class="fa fa-tags"></i> <b>'.$num_rooms.'</b></big> '.getAltText($texts['ROOM'], $texts['ROOMS'], $num_rooms).' 
        <big><i class="fa fa-male"></i> <b>'.$persons.'</b></big> '.getAltText($texts['PERSON'], $texts['PERSONS'], $persons).'<i class="fa fa-caret-right"></i>';
        ...
        <button name="book" id="btn-book" class="btn btn-success btn-lg btn-block mt5"><b>'.$texts['TOTAL'].' '.formatPrice($total*CURRENCY_RATE).'</b> <i class="fa fa-hand-o-right"></i> '.$texts['BOOK'].'</button>';
        ...
        <button name="request" class="btn btn-default btn-lg btn-block mt5"><i class="fa fa-comment"></i> '.$texts['MAKE_A_REQUEST'].'</small></button>';
    }
