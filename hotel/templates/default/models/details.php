        ...
        if(isset($_POST['book'])){
            ...
            if(isset($_SESSION['book']['activities']) && count($_SESSION['book']['activities']) > 0){
                foreach($_SESSION['book']['activities'] as $id_activity => $activity){
                    $activity_content .= '<p><b>'.$activity['title'].'</b> - '.$activity['duration'].' - '.strftime(DATE_FORMAT.' '.TIME_FORMAT, $activity['session_date']).'<br>
            ...
            $mail = getMail($db, 'BOOKING_REQUEST', array(
                ...
                '{Check_in}' => strftime(DATE_FORMAT, $_SESSION['book']['from_date']),
                '{Check_out}' => strftime(DATE_FORMAT, $_SESSION['book']['to_date']),
                    ...
                    <div class="col-md-6">
                        ...
                        if($user_id == 0){ ?>
                            <fieldset>
                                ...
                                <div class="form-group row">
                                    <div class="col-sm-12 text-right">
                                        <a href="#" class="btn btn-primary sendAjaxForm" data-action="<?php echo getFromTemplate('common/register/signup.php'); ?>"><i class="fa fa-power-off"></i> <?php echo $texts['SIGN_UP']; ?></a>
                                    </div>
                                </div>
                            </fieldset>
                            <?php
                        ...
                        if($result_service !== false && $db->last_row_count() > 0){ ?>
                            <fieldset class="mb20">
                                ...
                                foreach($result_service as $i => $row){
                                    ...
                                    if($service_type == 'night') $service_price *= $nb_rooms;
                                    ...
                                    <div class="row form-group">
                                        <label class="col-sm-<?php echo (strpos($service_type, 'qty') !== false) ? 7 : 10; ?> col-xs-9">
                                            <input type="checkbox" name="extra_services[]" value="<?php echo $id_service; ?>" class="sendAjaxForm" data-action="<?php echo getFromTemplate('common/update_booking.php'); ?>" data-target="#total_booking"<?php echo $checked;?>>
                                            <?php
                                            echo $service_title;
                                            if($service_descr != ''){ ?>
                        ...
                        if(isset($_SESSION['book']['amount_rooms'])){ ?>
                            <fieldset class="mb20">
                                ...
                                <div class="form-group form-inline">
                                    ...
                                    <a href="#" class="btn btn-primary sendAjaxForm" data-action="<?php echo getFromTemplate('common/update_booking.php'); ?>" data-target="#total_booking"><i class="fa fa-check"></i></a>
                                </div>
                            </fieldset>
                            <hr>
                            <?php
