            ...
            if($payment_type == 'paypal'){ ?>
                <div class="text-center">
                    ...
                    <form action="https://www.<?php if(PAYMENT_TEST_MODE == 1) echo 'sandbox.'; ?>paypal.com/cgi-bin/webscr" method="post">
                        ...
                        <input name="item_name" type="hidden" value="<?php echo addslashes(strftime(DATE_FORMAT, $_SESSION['tmp_book']['from_date']).' > '.strftime(DATE_FORMAT, $_SESSION['tmp_book']['to_date']).' - '.$_SESSION['tmp_book']['nights'].' '.$texts['NIGHTS'].' - '.($_SESSION['tmp_book']['adults']+$_SESSION['tmp_book']['children']).' '.$texts['PERSONS']); ?>">
                ...
                <div class="text-center">
                    ...
                    <form action="https://<?php if(PAYMENT_TEST_MODE == 1) echo 'sandbox'; else echo 'www'; ?>.2checkout.com/checkout/purchase" method="post">
                        ...
                        <input type="hidden" name="li_0_name" value="<?php echo addslashes(strftime(DATE_FORMAT, $_SESSION['tmp_book']['from_date']).' > '.strftime(DATE_FORMAT, $_SESSION['tmp_book']['to_date']).' - '.$_SESSION['tmp_book']['nights'].' '.$texts['NIGHTS'].' - '.($_SESSION['tmp_book']['adults']+$_SESSION['tmp_book']['children']).' '.$texts['PERSONS']); ?>">
